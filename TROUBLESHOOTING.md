...
### `scripts/test_gemini_connection.py` says "GOOGLE_API_KEY is not set"
- Check `.env` exists: `ls -la .env` (note the leading dot)
- Check it's not still the placeholder: `grep GOOGLE_API_KEY .env`
- Should show `GOOGLE_API_KEY=AIza...` not `your_google_ai_studio_key_here`

### `403 PERMISSION_DENIED` or `API key not valid`
- Your key might be malformed (extra spaces/quotes when pasting). Open
  `.env` in a plain text editor and confirm the line looks exactly like:
  `GOOGLE_API_KEY=AIzaSyXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX`
  (no quotes, no trailing spaces)
- Try regenerating the key at https://aistudio.google.com/apikey

### `429 RESOURCE_EXHAUSTED` (rate limit)
- Free tier has per-minute limits. Wait 60 seconds and retry.
- If it keeps happening during a full pipeline run (which makes ~15
  Gemini calls), that's expected on the free tier under heavy testing.
  Space out your test runs by a minute or two.

### `SSL: CERTIFICATE_VERIFY_FAILED`
This should NOT happen on a personal laptop with no corporate proxy.
If it does:
- Confirm you're not still connected to any corporate VPN
- Try: `pip install --upgrade certifi` inside the venv:
  `.venv/bin/python -m pip install --upgrade certifi`
- Check system date/time is correct (wildly wrong clocks break TLS)

### Gemini call hangs forever / times out
- Check your internet connection
- Try the connectivity script in isolation first (it has smaller
  payloads than the full pipeline):
...
### Nano Banana image generation fails: "did not return any image bytes"
- The model occasionally refuses to generate an image for certain
  prompts (safety filters). Try a different competitor brand or a
  more neutral `industry_hint`.
- Confirm `GEMINI_IMAGE_MODEL=gemini-2.5-flash-image-preview` in `.env`
  matches exactly (typos here silently fail).
...
uv sync --python 3.12
cp .env.example .env
# edit .env: paste GOOGLE_API_KEY, optionally GCP_PROJECT_ID + DATA_SOURCE
.venv/bin/python scripts/test_gemini_connection.py
